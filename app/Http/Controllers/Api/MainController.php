...
class MainController extends Controller
{
    ...
    public function cities()
    {
        ...
        return jsonResponse(1, 'success', CityResource::collection($cities));
    }

    public function regions(RegionRequest $request)
    {
        $regions = $this->mainService->getRegions($request->city_id);
        return jsonResponse(1, 'success', RegionResource::collection($regions));
    }

    public function restaurants()
    {
        $restaurants = $this->mainService->getRestaurants();
        return jsonResponse(1, 'success', RestaurantResource::collection($restaurants)->response()->getData(true));
    }

    public function foods(RestaurantRequest $request)
    {
        $foods = $this->mainService->getFoods($request->restaurant_id);
        return jsonResponse(1, 'success', ProductResource::collection($foods)->response()->getData(true));
    }
    ...
    public function restaurant(RestaurantRequest $request)
    {
        ...
        return jsonResponse(1, 'success', new RestaurantResource($restaurant));
    }

    public function comments()
    {
        $comments = $this->mainService->getComments();
        return jsonResponse(1, 'success', CommentResource::collection($comments)->response()->getData(true));
    }
    ...
    public function categories()
    {
        ...
        return jsonResponse(1, 'success', CategoryResource::collection($categories));
    }

    public function contacts(ContactRequest $request)
    {
        $contact = $this->mainService->createContact($request->validated());
        return jsonResponse(1, 'success', new ContactResource($contact));
    }
}
