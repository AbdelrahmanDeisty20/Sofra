...
class OrderController extends Controller
{
    ...
    public function newOrder(NewOrderRequest $request)
    {
        ...
        if ($result['status'] == 0) {
            return jsonResponse(0, $result['msg']);
        }
        return jsonResponse(1, $result['msg'], new OrderResource($result['order']));
    }

    public function myOrders(Request $request)
    {
        $orders = $this->orderService->getMyOrders($request->user());
        return jsonResponse(1, 'success', OrderResource::collection($orders)->response()->getData(true));
    }
    ...
    public function showOrder(Request $request)
    {
        ...
        if (!$order) {
            return jsonResponse(0, 'الطلب غير موجود');
        }
        return jsonResponse(1, 'success', new OrderResource($order));
    }

    public function cancelOrder(Request $request)
    {
        $result = $this->orderService->cancelOrder($request->user(), $request->order_id);
        return jsonResponse($result['status'], $result['msg']);
    }
